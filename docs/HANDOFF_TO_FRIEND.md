...
Commit 16D completes the deterministic post-conflict path without reactivating the old Challenge. `openInvestigationConflictContinuation(...)` creates a distinct immutable Challenge partition whose answer anchor is the exact validated 14B2 result revision and whose server-derived trigger Evidence is the conflict application's exact applied Evidence. A central `APPLIED_CHALLENGE_CONFLICT` resolver and shared historical application validation prove the conflict application, source/result revision chain, permanent Question/product ownership, exact applied artifact allowlists, and unchanged operational-only material state. Visited `positive:<ref>` / `conflict:<ref>` identities fail closed on recursive provenance.

Only applied conflict Claims, the applied contradiction Assessment, and applied conflict Evidence cross into continuation analysis as historical provenance. Reviewed-only source artifacts remain excluded. New Requests, Claims, Assessments, and Establishment rows stay associated with the continuation Challenge; no historical association moves, and supersession cannot cross the conflict boundary. Historical conflict Assessments remain `MATERIAL_REVISION_STALE`. New current-material Assessments can reason over allowlisted historical conflict Claims and current continuation Claims. Evidence remains Question-owned, so Evidence received late from an old Request can be current without currentifying or reopening the old Request.

The existing Challenge batch Establishment/application subsystems now select v3 only for `APPLIED_CHALLENGE_CONFLICT`. v1 first-cycle and v2 positive-inherited meanings remain frozen. v3 is `RESOLVE_CONFLICT`: a fresh continuation Claim requires current complete-corpus HUMAN support, current trusted rejection of divergent Claims, source diversity, and a clean current analysis. The HUMAN application policy stays v1 and caller authority is unchanged. Its positive result baseline contains only the fresh target Claim, qualifying fresh SUPPORT, and complete current Question Evidence; old conflict Claims, the historical contradiction, current REJECT justification, and reviewed-only artifacts do not leak into the next Commit 16C cycle.

Commit 16D intentionally does not support a second authoritative conflict transition while already conflicted; investigation continues in the same continuation partition until a qualifying HUMAN positive application returns the case to `KNOWN/BATCH_LOT`. It adds no schema or migration, no public route/UI, no HumanDecision authorization surrogate, no generic `ACCEPT_INVESTIGATION` path, and no AI final authority. Commit 17 monitoring/AI alert provenance hardening is next; a constrained AI Investigator remains later work.

Commit 16D verification: the focused applied-conflict suite passes 18/18; the adjacent Investigation Engine batch passes 141/141 in 8 files; lifecycle/Review/Exposure/Task/Closure regressions pass 62/62 in 8 files; and the full suite passes 372/372 in 36 files. Static checks report 0 errors/warnings, the production build passes with only the existing adapter-auto notice, tracked/untracked whitespace checks pass, Drizzle generation is a no-op, and no 0016 or 0017 artifact exists.

Commit 17 removes the monitoring trust conflation. Monitoring first records the exact immutable raw JSON as an `alert_source_observations` node, then stores structured and derived values as field-level `alert_field_assertions`. AI extraction has a strict five-field proposal schema and persists only `AI_PROPOSAL` assertions; it cannot replace source identity, URL/time, title, description, or risk. Missing credentials or model failure means zero proposals, not a fabricated deterministic extraction.

All consequential consumers go through `resolveAlertFactsInTransaction(...)`. DISCOVERY may use AI proposals as attributed ranking hints, but AUTHORITATIVE GTIN and batch policy excludes both AI and legacy/unverified values. GTIN-8/12/13/14 check digits are validated centrally. A generic product-match confirmation never promotes proposal fields and there is intentionally no production `HUMAN_CONFIRMED` writer without an independent durable pre-case evidence source.

Each new Match has one immutable `alert_match_bases` record binding the exact observation, field assertion sets, catalogue product snapshot, scoring policy, and explanation provenance. New Review confirmation rehydrates that basis in the same transaction, rejects source or catalogue staleness, and builds `InvestigationOutcome` only from attributed authoritative facts. Existing pre-Commit-17 rows are preserved with no invented source provenance or trusted backfill; a new decision over legacy-only data requires re-ingestion. Historical Review/Case replay remains before present-day authorization.

Migration `0016_simple_carlie_cooper.sql` adds only `alert_source_observations`, `alert_field_assertions`, and `alert_match_bases` plus restrictive indexes/checks/FKs. The local archive and seeds are explicitly synthetic `demo_archive` observations with `demo:true`; there is still no live Safety Gate connector. Commit 18 remains limited to a constrained AI Investigator that consumes these trust boundaries. It must not promote `AI_PROPOSAL`, select consequential actions, or bypass existing RULE/HUMAN authority.

Commit 17 verification: focused provenance/monitoring/matching/Review checks pass 63/63 in 9 files; the complete Investigation regression suite passes 246/246 in 15 files; downstream lifecycle/Review/Exposure/Task/Closure checks pass 64/64 in 9 files; and the full suite passes 391/391 in 39 files. Static checks report 0 errors/warnings, the production build passes with only the existing adapter-auto notice, tracked/untracked whitespace checks pass, empty and populated-0015 migration coverage passes with `foreign_key_check = []`, and final Drizzle generation is a no-op.

Commit 18 adds constrained next-step recommendations without extending AI authority. `investigator-snapshot.ts` selects the exact permanent Question and current Investigation partition, then serializes validated Investigation and Commit 17 provenance with explicit epistemic classes. It never treats `AI_PROPOSAL` or `LEGACY_UNVERIFIED` as fact, never reads raw alert EAN/batch as authority, and sends only bounded Evidence metadata and allowlisted operational summaries rather than raw payloads, documents, recipients, or customer data.

The model can choose only one closed advisory action: request one of the three existing evidence types from the abstract current-product supplier, review exact existing Evidence, follow up an exact recorded request attempt, escalate unresolved work to a human, or take no action at a resolved/HUMAN-only gate. WAIT is deliberately disabled because `evidence_requests.status = pending` proves only a recorded attempt. No recommendation writes an Evidence Request, Task, Claim, Assessment, HumanDecision, Establishment, application, case state, external message, or stock action.

`investigator.ts` uses two transactions around the remote call. The first builds and fingerprints the exact basis and replays an existing immutable recommendation. The OpenAI call runs with no database transaction. The second IMMEDIATE transaction rebuilds the snapshot and inserts only if the complete digest is unchanged; stale responses persist nothing and concurrent same-basis inserts hydrate the winner. `DISMISSED`, `ACTED`, and `PATH_EXHAUSTED` are separate immutable HUMAN/demo-operator events, with exact artifact ownership and durable Evidence required where applicable. There is no mutable recommendation status.

The separate server-only client uses strict structured output, no tools, `store:false`, bounded output, and a versioned prompt that treats all supplied material as untrusted data. Missing `OPENAI_API_KEY` returns typed unavailable rather than a fake AI fallback. CI uses mocks and makes no live model call. Migration `0017_red_nocturne.sql` adds only the two append-only ledgers and no historical backfill. Commit 18 has no route/UI or browser contract; optional presentation wiring and a controlled live-model smoke test are the only remaining demonstration work.

Commit 18 verification: focused snapshot/policy/service/client/migration checks pass 23/23 in 5 files; all Investigation tests pass 264/264 in 18 files; Commit 17 regressions pass 41/41 in 7 files; downstream lifecycle regressions pass 64/64 in 9 files; and the full suite passes 414/414 in 44 files. Static checks report no diagnostics, the production build passes with only the existing adapter-auto notice, whitespace checks pass, migration coverage preserves populated 0016 state with `foreign_key_check = []`, and final Drizzle generation is a no-op.

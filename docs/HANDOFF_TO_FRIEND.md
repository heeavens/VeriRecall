...
Migrations present are `0000_initial.sql`, `0001_last_living_lightning.sql`, `0002_case_lifecycle.sql`, `0003_traceability_exposure.sql`, `0004_last_thunderbolt.sql`, `0005_majestic_centennial.sql`, `0006_cynical_rictor.sql`, `0007_calm_captain_cross.sql`, additive `0008_warm_zarek.sql`, additive `0009_glamorous_celestials.sql`, and additive `0010_lying_hellcat.sql`. The current Commit 11 baseline passes 256 tests in 30 files, including focused Challenge/clean-repeat/populated-0009 migration coverage, SQLite constraints, and `foreign_key_check`. Re-run current checks after future work because a historical green baseline is not proof that a later diff is correct.
...
New Evidence must resolve through that registry as well as existing request ownership. Direct late Evidence remains recordable after `BATCH_MISSING` disappears, but remains source material only: it does not recreate a Gap, create a Claim/Assessment/Challenge, or increment case/material versions. `readInvestigationQuestionContext` says only `OPEN_GAP` or `NOT_CURRENT`; no direct question-to-resolution linkage exists yet. Requests, Claims, Assessments, Effective Analysis, and Establishment remain blocked after resolution.
...
Commit 12 should add Challenge-scoped Evidence Requests, Claims, Assessments, and Effective Analysis using additive association persistence rather than rebuilding the populated Commit 5–9 tables. Those services remain `OPEN_GAP`-only today, and Commit 11 does not change Establishment policy or authoritative scope. Do not implement authoritative batch resolution until challenge analysis and explicit re-review application are safe. The monitoring provenance conflation described below also remains unresolved.

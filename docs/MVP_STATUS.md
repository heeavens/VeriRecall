...
- Challenge opening uses the fixed `demo_operator` HUMAN identity, appends one chronology audit event, and writes no other state. It is allowed while the case is CLOSED and does not reopen it. Requests, Claims, Assessments, Effective Analysis, and Establishment remain current-`BATCH_MISSING` only; Commit 12 is expected to add Challenge-scoped investigation writes and reuse the effective-analysis graph without changing authoritative scope.
- Verification: the focused Challenge/database suites pass 22/22 and the full suite passes 256/256 in 30 files. Type/check, build, migration metadata, and diff verification results are recorded in the current handoff after the final completion gate.

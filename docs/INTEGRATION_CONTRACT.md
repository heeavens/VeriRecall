...
| --- | --- | --- |
| ACCEPT_INVESTIGATION | outcome | Только internal lifecycle plumbing с opaque server capability; публичный `/commands` всегда возвращает FORBIDDEN до mutation |
...
Схема по-прежнему парсит внутренний `ACCEPT_INVESTIGATION`, включая `expectedCaseVersion=0`, но публичный runtime не даёт браузеру authority выполнить эту команду ни для зарезервированного, ни для существующего case. Initial authoritative outcome создаётся выделенным server-side Review bridge. Ни версия, ни содержимое outcome, ни demo/actor поля клиента не создают internal capability.
...
4. Для явно авторизованного внутреннего ACCEPT_INVESTIGATION более низкая materialRevision → STALE_INVESTIGATION; та же revision с отличающимся outcome → IDEMPOTENCY_CONFLICT. Тот же outcome/revision с новым commandId и актуальной expectedCaseVersion — no-op с replayed=true, без увеличения версии. Более высокая revision принимается после проверки принадлежности; пропуски номеров допустимы. Публичный контекст останавливается с FORBIDDEN до этих правил.
...
- Reservation для существующего alert/product создаёт case и snapshot **version 1**, investigation/materialRevision=null, без case_items и фиктивных количеств. `ACCEPT_INVESTIGATION` остаётся schema/internal-plumbing формой, но не публичным следующим шагом: initial Review использует отдельный `applyConfirmedReviewOutcomeInTransaction(...)`. `scripts/demo-lifecycle.ts` теперь требует `VERIRECALL_CASE_ID` уже инициализированного через Review case и отправляет только разрешённые downstream-команды.
- Публичный контекст остаётся explicit demo для разрешённых команд: VERIRECALL_DEMO_MODE=true, actor demo_operator, role CASE_MANAGER, demo:true. Это не даёт authority для `ACCEPT_INVESTIGATION`: она требует отдельной opaque server capability, которой нет у HTTP context. Этап 6 уже записывает решения, но не меняет factual knowledge и не удаляет gaps/conflicts.
...
- Для изменения live состояния используются только новые команды. Старые команды для lifecycle case заблокированы. Этап 7 добавил единственное узкое исключение на входе: подтверждение Review может один раз дополнить уже подтверждённое однопродуктовое legacy case начальным versioned snapshot. Старые строки не удаляются и не становятся источником versioned task/closure state; несовпадающий product и multi-product conversion отвергаются.

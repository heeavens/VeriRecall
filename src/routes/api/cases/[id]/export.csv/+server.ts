...
export const GET: RequestHandler = async ({ params }) => {
  ...
  try {
    const bytes = await new CaseReportExporter(db).exportCase(caseId.data, 'csv');
    return new Response(new Uint8Array(bytes).buffer, {
      headers: {
        'content-type': 'text/csv; charset=utf-8',
        'content-disposition': `attachment; filename="recall-case-${caseId.data}.csv"`,
        'cache-control': 'no-store'
      }
    });
  } catch (reportError) {
    if (reportError instanceof CaseReportError) error(404, reportError.message);
    error(500, 'The CSV report could not be generated. Please retry.');
  }
};
